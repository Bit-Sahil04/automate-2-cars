...
def is_car_on_lane(lane_number: int):
    ...
    car_heights = (
        car_top_point + game_footer_offset,
        car_mid_point + game_footer_offset,
        car_bottom_point + game_footer_offset,
    )
...
def count_pixels_on_lane_y(lane_number, height):
    ...
    is_background = (
        lambda x: color_diff(x, background) < color_threshold
        or color_diff(x, dark_bg) < color_threshold
    )
...
def detect_shape_on_lane(lane_number):
    x_start = (
        window_x
        + game_offset_x
        + lane_divider_width[lane_number]
        + lane_width * lane_number
    )
    height = 130

    region = (x_start, 527, lane_width, height)
    # region=(1502,527,98,74)

    circle = pyautogui.locateCenterOnScreen(
        "circle_blue.png", confidence=0.8, grayscale=True, region=region
    )
    square = pyautogui.locateCenterOnScreen(
        "square_blue.png", confidence=0.8, grayscale=True, region=region
    )
...
with mss.mss() as sct:
    ...
    region = {
        "top": game_header_offset,
        "left": 1492,
        "width": 430,
        "height": 964 - game_header_offset - game_footer_offset,
    }
